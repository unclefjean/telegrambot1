...
async def handle_document(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if document.mime_type == "application/pdf":
            ...
            await update.message.reply_text(f"Файл обработан и добавлен в список для объединения. Используйте /combine для объединения.")
        elif document.mime_type == "application/zip":
            extracted_files = extract_zip(temp_file_path)
            await update.message.reply_text(f"ZIP-архив обработан, добавлено {len(extracted_files)} файлов. Используйте /combine для объединения.")
        else:
